// ─────────────── page-tires.jsx — Tires PLP + PDP components
const { useState, useEffect, useMemo, useRef } = React;

function TiresPLP() {
  const { vehicle, navigate, tireBrands: brands } = useApp();
  const [tires, setTires] = useState(window.HOLBROOK_DATA.tires);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    setLoading(true);
    api.fetchTires().then(data => {
      const items = extractItems(data);
      if (items) {
        const normalized = items.map(function(t) {
          return {
            id: t.Id || t.PartNumber || ('t-' + (t.Brand||'') + '-' + (t.Model||'') + '-' + (t.Size||'')).replace(/\s/g,'-').toLowerCase(),
            brand: t.Brand || t.brand || '',
            model: t.Model || t.model || '',
            size: t.Size || t.size || '',
            season: (t.Tags && t.Tags.SegmentTags && t.Tags.SegmentTags[0]) || (t.Season || t.season || 'All-Season'),
            load: t.Specs && t.Specs.LoadIndex ? ('' + t.Specs.LoadIndex + (t.Specs.SpeedRating || '')) : (t.LoadIndex || ''),
            warranty: t.Warranty || 0,
            priceEach: t.Price || t.price || 0,
            rating: t.Rating || (4.5 + Math.random() * 0.4),
            reviews: t.Reviews || Math.floor(100 + Math.random() * 500),
            runFlat: t.RunFlat || false,
            imgUrl: t.ImgUrlBase && t.ImgThumb ? t.ImgUrlBase + t.ImgThumb : (t.imgUrl || null),
          };
        });
        console.log('[Holbrook] Loaded', normalized.length, 'tires from API');
        setTires(normalized);
        window.HOLBROOK_DATA.tires = normalized;
      } else {
        console.warn('[Holbrook] Tires: no items found. Keys:', data ? Object.keys(data) : 'null');
      }
    }).catch(function(err) {
      console.warn('[Holbrook] Tires API failed, using static fallback:', err.message || err);
  ...
  const initSeason = params.get('season') || '';
  const [seasonF, setSeasonF] = useState(initSeason ? [initSeason] : []);
  const [brandF, setBrandF]   = useState([]);
  const [sizeF, setSizeF]     = useState('');
  const [sort, setSort]       = useState('featured');

  const seasons = ['Summer','All-Season','All-Weather','Winter','All-Terrain','UHP All-Season'];
  const sizes   = useMemo(() => [...new Set(tires.map(t => t.size))].sort(), [tires]);

  const filtered = useMemo(() => {
    let L = tires;
    if (seasonF.length) L = L.filter(t => seasonF.includes(t.season));
    if (brandF.length)  L = L.filter(t => brandF.includes(t.brand));
    if (sizeF)          L = L.filter(t => t.size === sizeF);
    ...
    return L;
  }, [tires, seasonF, brandF, sizeF, sort]);
  ...
  return (
    <div style={{ maxWidth: 1320, margin:'0 auto', padding:'40px 32px' }}>
      <div style={{ borderBottom:`1px solid ${tokens.line}`, paddingBottom: 24, marginBottom: 28 }}>
        <Eyebrow>Shop tires</Eyebrow>
        <Headline size="h1" style={{ fontSize: 56, marginTop: 6 }}>
          {vehicle ? `Tires that fit your ${vehicle.year} ${vehicle.make} ${vehicle.model}` : 'Replacement tires · all sizes'}
        </Headline>
        <p style={{ fontSize: 15, color: tokens.taupe, marginTop: 10, maxWidth: 720 }}>
          Michelin, Bridgestone, Goodyear, Continental and more. Same-day mount at any of our three Detroit-area shops.
        </p>
      </div>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'260px 1fr', gap: 40 }}>
        <aside>
          <FilterGroup label="Season">
            {seasons.map(s => (
              <FilterCheck key={s} checked={seasonF.includes(s)} onChange={()=>toggle(seasonF,setSeasonF,s)} label={s} />
            ))}
          </FilterGroup>
          <FilterGroup label="Brand">
            {brands.map(b => (
              <FilterCheck key={b.name} checked={brandF.includes(b.name)} onChange={()=>toggle(brandF,setBrandF,b.name)} label={b.name} />
            ))}
          </FilterGroup>
          <FilterGroup label="Size">
            <select value={sizeF} onChange={e=>setSizeF(e.target.value)} style={{ width:'100%', padding:'10px 12px', fontSize: 13, fontWeight: 600, border:`1px solid ${tokens.line}`, borderRadius: 6, background:'#fff' }}>
              <option value="">All sizes</option>
              {sizes.map(s => <option key={s}>{s}</option>)}
            </select>
          </FilterGroup>
          {(seasonF.length || brandF.length || sizeF) ? (
            <button onClick={()=>{ setSeasonF([]); setBrandF([]); setSizeF(''); }} style={{
        ...
        <div>
          <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: 16 }}>
            <div style={{ fontSize: 13, color: tokens.taupe, fontWeight: 600 }}>{filtered.length} tires</div>
          ...
          {loading && (
            <div style={{ padding: 60, textAlign:'center' }}>
              ...
              <div style={{ marginTop: 14, fontSize: 13, color: tokens.mute, fontWeight: 600 }}>Loading tires from inventory...</div>
              <style>{`@keyframes htmSpin { to { transform: rotate(360deg); } }`}</style>
            </div>
          )}
          {!loading && (
            <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill, minmax(280px, 1fr))', gap: 18 }}>
              {filtered.map(t => <TireCard key={t.id} tire={t} />)}
            </div>
          )}
          {!loading && filtered.length === 0 && (
            <div style={{ padding: 40, textAlign:'center', color: tokens.taupe }}>
              No tires match those filters. <button onClick={()=>{ setSeasonF([]); setBrandF([]); setSizeF(''); }} style={{ background:'none', border:'none', color: tokens.primary, fontWeight: 700, cursor:'pointer', textDecoration:'underline' }}>Clear filters</button>
            </div>
          )}
        </div>
      </div>
...
function TireCard({ tire }) {
  ...
  return (
    <a href={`/tires/?product=${tire.id}`} onClick={e=>{e.preventDefault(); navigate(`/tire/${tire.id}`);}} style={{
    ...
    onMouseOut={e => { e.currentTarget.style.borderColor = tokens.line; e.currentTarget.style.transform = 'none'; }}>
      <div style={{ aspectRatio:'4/3', background: tokens.surfLow, borderRadius: 4, display:'flex', alignItems:'center', justifyContent:'center', marginBottom: 14 }}>
        {tire.imgUrl ? (
          <img src={tire.imgUrl} alt={tire.brand + ' ' + tire.model} style={{ maxWidth:'85%', maxHeight:'85%', objectFit:'contain' }} />
        ) : (
          <TireSvg />
        )}
      </div>
      <div style={{ fontSize: 11, color: tokens.mute, fontWeight: 700, letterSpacing:'.14em', textTransform:'uppercase' }}>{tire.brand} · {tire.season}</div>
      <div style={{ fontFamily:'Space Grotesk', fontSize: 17, fontWeight: 700, marginTop: 4, lineHeight: 1.25 }}>{tire.model}</div>
      <div style={{ fontFamily:'JetBrains Mono', fontSize: 12, color: tokens.graphite, marginTop: 4 }}>{tire.size}</div>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginTop: 12, paddingTop: 12, borderTop:`1px solid ${tokens.lineSoft}` }}>
        <div>
          <div style={{ fontSize: 10, color: tokens.mute, fontWeight: 700, letterSpacing:'.14em', textTransform:'uppercase' }}>Per tire</div>
          <div style={{ fontFamily:'Space Grotesk', fontSize: 22, fontWeight: 800, color: tokens.primary, marginTop: 2 }}>${tire.priceEach}</div>
          <div style={{ fontSize: 11, color: tokens.taupe, marginTop: 2 }}>${tire.priceEach*4} · set of 4</div>
        </div>
        <Stars rating={tire.rating} reviews={tire.reviews} />
      </div>
    </a>
  );
}

function TirePDP({ productId }) {
  const { tires } = window.HOLBROOK_DATA;
  const { navigate, addToCart } = useApp();
  const id = productId;
  const tire = tires.find(t => t.id === id) || tires[0];
  const [qty, setQty] = useState(4);
  const [showPkgPrompt, setShowPkgPrompt] = useState(false);

  const handleAdd = () => {
    const item = {
      id: tire.id, type: 'tire', qty,
      name: `${tire.brand} ${tire.model}`,
      subtitle: `${tire.size} · ${tire.season}`,
      price: tire.priceEach,
      rawBrand: tire.brand, rawModel: tire.model, size: tire.size,
    };
    addToCart(item, qty);
  ...
  return (
    <div style={{ maxWidth: 1320, margin:'0 auto', padding:'40px 32px' }}>
      <button onClick={()=>navigate('/tires')} style={{ background:'none', border:'none', color: tokens.taupe, fontSize: 12, fontWeight: 700, letterSpacing:'.14em', textTransform:'uppercase', cursor:'pointer', padding: 0, marginBottom: 20 }}>
        &#8592; Back to tires
      </button>
      ...
      <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap: 48 }}>
        <div style={{ background: tokens.surfLow, borderRadius: 8, aspectRatio:'1/1', display:'flex', alignItems:'center', justifyContent:'center' }}>
          {tire.imgUrl ? (
            ...
            <div style={{ width:'70%' }}><TireSvg /></div>
          )}
        </div>

        <div>
          <div style={{ fontSize: 11, color: tokens.mute, fontWeight: 700, letterSpacing:'.18em', textTransform:'uppercase' }}>{tire.brand} · {tire.season}</div>
          <Headline size="h1" style={{ fontSize: 44, marginTop: 6 }}>{tire.model}</Headline>
          <div style={{ fontFamily:'JetBrains Mono', fontSize: 16, color: tokens.graphite, marginTop: 8 }}>{tire.size} · {tire.load}</div>
          <div style={{ marginTop: 14 }}><Stars rating={tire.rating} reviews={tire.reviews} /></div>
          ...
          <div style={{ display:'flex', gap: 24, marginTop: 24, padding:'18px 0', borderTop:`1px solid ${tokens.line}`, borderBottom:`1px solid ${tokens.line}` }}>
            <SpecItem label="Per tire" value={`$${tire.priceEach}`} big />
            <SpecItem label="Set of 4" value={`$${tire.priceEach*4}`} big />
            {tire.warranty > 0 && <SpecItem label="Mileage warranty" value={`${(tire.warranty/1000)}k mi`} />}
          </div>
            ...
            <div style={{ display:'flex', gap: 8 }}>
              {[1,2,4].map(n => (
                <button key={n} onClick={()=>setQty(n)} style={{
                  padding:'12px 24px', fontFamily:'Space Grotesk', fontWeight: 700, fontSize: 14,
          ...
          <div style={{ marginTop: 20, padding: 18, background: tokens.surfLow, borderRadius: 6, display:'flex', gap: 12, alignItems:'flex-start' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 13, color: tokens.taupe, marginTop: 2, lineHeight: 1.45 }}>Schedule install at any Holbrook location. Finalize the balance when you drop off your vehicle.</div>
            </div>
          </div>
        </div>
      </div>

      {showPkgPrompt && <BuildPackagePrompt type="tire" item={tire} onClose={()=>{ setShowPkgPrompt(false); navigate('/cart'); }} />}
    </div>
  );
}
...
Object.assign(window, { TiresPLP, TirePDP });
